...
class WebAnnounce(commands.Cog):
    ...
    @staticmethod
    def is_message_recent(message: discord.Message, minutes: int = 240) -> bool:
    ...
    async def check_for_webhook_updates(self):
        ...
        if not self.is_message_recent(message):
            await message.channel.send(f"There hasn't been any messages in 4 hours <@234515446659940353>")
async def setup(bot):
